...
class Diffuse:
    def __init__(
        ...
        den = data['den']
        kappa = data['kappa']
        self.kappa_t4 = interpolate.RectBivariateSpline(tem, den, kappa, kx=2, ky=2, s=1)
    ...
    def eps(self, T, density):
        ...
        return (5.3e21 * rho5 ** 2 * Y ** 3 / T8 ** 3) * np.exp(-44 / T8)  # in erg/g/s
        ...
        for j in range(self.Nz):
            for i in range(self.Nx):  # i.e. i = 1:79
                ...
                if i == 0:
                    ...
                    a_w = (
                        self.get_a_e(self.x[i], j)
                        / (self.rho(self.z[j]) * self.get_kappa(T[i,j],self.rho(self.z[j])) * self.dx)
                    )
                ...
                if j == 0:
                    ...
                    a_s = (
                        ...
                        / (self.rho(self.z[j]) * self.get_kappa(T[i,j],self.rho(self.z[j])) * self.dz)
                    )
                ...
                if j == self.Nz-1:
                    ...
                    if self.bb_flag == 'bb' and i not in hotspoty:
                        ...
                        / (self.rho(self.z[j + 1]) * self.get_kappa(T[i,j],self.rho(self.z[j])) * self.dz)
                        )
                ...
                if j == self.Nz-1:
                    ...
                    if i in hotspoty:
                        ...
                        if self.bb_flag == 'bb':
                            ...
                            a_n = 0.0
                            a_n_2 = -self.sigma * self.Ttop**4 * self.get_a_s(self.x[i+1], self.x[i])
                            print(f'F_ n = {a_n_2}')
                        else:
                            a_n = (self.get_a_s(self.x[i+1], self.x[i])
                            / (self.rho(self.z[j + 1]) * self.get_kappa(T[i,j],self.rho(self.z[j])) * self.dz)
                    ...
                    a_n = (self.get_a_s(self.x[i+1], self.x[i])/ ((self.rho(self.z[j + 1])) * self.get_kappa(T[i,j],self.rho(self.z[j])) * self.dz)
                    )
                    a_n_2 = a_n * T[i, j+1]
...
# what should Rin and Rout be for weighting for north and south points?? Total R of cylinder, or R of zone?
        for j in range(self.Nz):
            for i in range(self.Nx):  # i.e. i = 1:79
                ...
                if i == 0:
                    ...
                    a_w = (
                        self.get_a_e(self.x[i], j)
                        / (self.rhoz[j] * self.get_kappa(T[i,j],self.rhoz[j]) * self.dx)
                    )
                    A[p, self.m(i-1, j)] = a_w
                ...
                if j == 0:
                    ...
                    a_s = (
                        self.get_a_s(self.x[i+1], self.x[i])
                        / (self.rhoz[j] * self.get_kappa(T[i,j],self.rhoz[j]) * self.dz[j])
                    )
                else:
                    a_s = (
                        self.get_a_s(self.x[i+1], self.x[i])
                ...
                if j == self.Nz-1:
                    hotspoty = np.arange(0, int(self.hsize / 2))
                    if self.bb_flag == 'bb' and i not in hotspoty:
                        #a_n = self.sigma * self.Ttop**4 * self.get_a_s(self.x[i+1], self.x[i])
                        a_n = 0.0
                    else:
                        a_n = (self.get_a_s(self.x[i+1], self.x[i])
                        / (self.rhoz[j+1] * self.get_kappa(T[i,j],self.rhoz[j]) * self.dz[j])
                        )
        ...
        for j in range(self.Nz):
            for i in range(self.Nx):  # i.e. i = 1:79
                ...
                if j == self.Nz-1:
                    ...
                    a_n = (self.get_a_s(self.x[i+1], self.x[i])/ ((self.rhoz[j+1]) * self.get_kappa(T[i,j],self.rhoz[j]) * self.dz[j])
                    )
                    a_n_2 = a_n * T[i, j+1]
    ...
    def run(self):
        ...
        zplot, xplot = np.meshgrid(self.z,
            np.hstack((-self.x[1:][::-1], self.x)))
        ...
        plt.pcolormesh(xplot, zplot, np.log10(
                 np.power(T_p_in, 0.25)),cmap=colourMap)
        plt.axis("equal")
        # Set Colorbar
        cbar = plt.colorbar()
...
# -------------------------------------------------------------------------------------------------- #
        ...
        norm = Normalize(vmin=np.log10(np.power(T_p, 0.25)).min(), vmax=np.log10(np.power(T_p, 0.25)).max())
        f = plt.figure(figsize=(10, 5))
        ...
        cm = ax.pcolormesh(xplot, zplot,np.log10(np.power(T_p, 0.25)),cmap=colourMap) #np.log10(
        ...
        # )
        # plt.contourf(
        #     xplot[:-1,:-1], zplot[:-1,:-1], np.log10(np.power(T_p, 0.25)), levels = 50,
        #     cmap=colourMap,
        # )
        # plt.axis("equal")
        # # Set Colorbar
        ...
        for i in range(0,len(T_n[:,0])):
            for j in range(0,len(T_n[0,:])):
                ...
                kappa[i,j] = self.get_kappa(T_n[i,j], self.rhoz[j])
...
# -------------------------------------------------------------------------------------------------- #
        ...
        # # plot column depth:
        # # Configure the contour
        # yplot, xplot = np.meshgrid(ycol,
        #     np.hstack((-self.x[1:][::-1], self.x[1:])))
        ...
        #     + self.rhoflag
        # )
        # plt.pcolormesh(xplot, yplot, np.log10(
        #          np.power(T_p[1:,:], 0.25)),cmap=colourMap)
        # #plt.axis("equal")
        # # Set Colorbar
        # cbar = plt.colorbar()
        # cbar.set_label('log(T)')
    ...
    def read_file(self, run_name):
        ...
        with open(run_name + "_initialdata.txt") as f:
    ...
    def plot_from_save(self, run_name):
        ...
        Nx, Nz, Lx, Lz, Tcrust, Ttop, Thotspot, hsize, kappa, Fbase, x, z, T_n = self.read_file(
            run_name
        )
        # Set colour interpolation and colour map
        colorinterpolation = 50
