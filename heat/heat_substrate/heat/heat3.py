"""
3D heat transport module STARSPOT
"""
...
class Diffuse(object):
    def __init__(
        ...
        root4=True,
        Y=0.99,
        net='static',  # static | dynamic | fancy
        bc=None,
        logfac=0.1,
        kappa=(
            '~/python/heat/kappa.npz',
            '~/python/heat/kappa_fe.npz',
        ),  # <filename>(tuple) | value | None
        cp=(
            '~/python/heat/cp.npz',
            '~/python/heat/cp.npz_fe',
        ),  # <filename>(tuple) | value | None
        dump=None,
        run_name='test',
        substrate=True,
        ...
        if dump is not None:
            ...
            rhof1[1:-1] = sqrt(rhoc1[1:] * rhoc1[:-1])
            if j0 > 0:
                rhof1[0] = sqrt(rhoc1[j0] * rhoc1[j0 - 1])
            else:
                rhof1[0] = rhoc[0]
            if j1 >= dump.jm:
                rhof1[-1] = rhoc1[-1]
            else:
                rhof1[-1] = sqrt(rhoc1[j1] * rhoc1[j1 + 1])
            tc1 = dump.tn[sel]
            lz = zf1[-1] - zf1[0]
            compz1 = np.zeros(nz, dtype=np.int)
            compz1[:jsub] = 1
        else:
            compz1 = np.full(nz, 0)
        self.dump = dump
        ...
        if kappa == None:
            ...
            self._kappa = kappa
        self.const_kappa = not isinstance(self._kappa, (str, tuple,))

        if cp == None:
            self._cp = 2.5 * RK * (3 / 4)  # er/g/K ideal He4 gas
        else:
            self._cp = cp
        self.const_cp = not isinstance(self._cp, str)
        ...
        compz1f = np.append(compz1, compz1[-1])
        self.compc = np.tile(compz1, (nx, ny, 1))
        self.compx = np.tile(compz1, (nx1, ny, 1))
        self.compy = np.tile(compz1, (nx, ny1, 1))
        self.compz = np.tile(compz1f, (nx, ny, 1))

        if self.ncomp > 1:
            jsub = np.where(compz1 == 0)[0][0]
            self.net.ppn[:, :, :jsub, :] = 0.0
        ...
        #   v. - 3D space vectors for plotting
        ...
        if geo in ('d', 'cart', 'cartesian',):
            ...
            if p0 < 0.25:
                h = (1 - np.cos(xf1 * r1i + a0)).reshape(-1, 1, 1) * rzf1
            else:
                h = np.sin(xf1 * r1i + (ph - a0)).reshape(-1, 1, 1) * rzf1
            fz = h * (r + zf1).reshape(1, 1, -1) * p2 * dy1.reshape(1, -1, 1)
            az[...] = fz[1:, :, :] - fz[:-1, :, :]

            w = np.sin(xf1 * r1i + a0).reshape(-1, 1, 1) * rzf1
        ...
        # surface coordinates used to locating spots
        self.xcsurf = 0.25 * (
        ...
        if self.geo in ('cartesian', 'patch',):
            self.rcsurf = sqrt(self.xcsurf ** 2 + self.ycsurf ** 2)
    ...
    def kappai_func(self, T, rho, comp, T4=True):
        if self.const_kappa:
        ...
        if not hasattr(self, 'kappa_data'):
            if self.ncomp > 1:
                assert isinstance(self._kappa, tuple)
                assert len(self._kappa) >= self.ncomp
            elif isinstance(self._kappa, tuple):
                self._kappa = self._kappa[0]
            self.kappa_data = kappa(self._kappa)

        # breakpoint()

        if self.ncomp > 1:
            kappai, kappait = self.kappa_data(T, rho, table=comp, T4=T4, drho=False)
        else:
            kappai, kappait = self.kappa_data(T, rho, T4=T4, drho=False)
        return kappai, kappait

    def cp_func(self, T, rho, comp, T4=True):
        if self.const_cp:
            if not hasattr(self, 'cp'):
                self.cp = (self._cp, np.array(0.0))
            return self.cp

        if not hasattr(self, 'cp_data'):
            if self.ncomp > 1:
                assert isinstance(self._cp, tuple)
                assert len(self._cp) >= self.ncomp
            elif isinstance(self._cp, tuple):
                self._cp = self._cp[0]
            self.cp_data = cp_data(self._cp)
        if self.ncomp > 1:
            cp, cpt = self.cp_data(T, rho, table=comp, T4=T4, drho=False)
        else:
            cp, cpt = self.cp_data(T, rho, T4=T4, drho=False)
    ...
    def ihot_func(self, reset=False):
        ...
        if self.surf == 'x':
            ...
            f = a / (self.xhot ** 2 * GOLDEN)
            n = 1 + np.random.randint(1 + sqrt(f))
    ...
    def func(self, T_, dt=None, T4=True):
        ...
        kix, kixt = self.kappai_func(Tfx, self.rhox[iciaa], self.compx[iciaa], T4=T4)
        kiy, kiyt = self.kappai_func(Tfy, self.rhoy[icaia], self.compy[icaia], T4=T4)
        kiz, kizt = self.kappai_func(Tfz, self.rhoz[icaai], self.compz[icaai], T4=T4)
        ...
        if self.bc[0][0] == REFLECTIVE:
            ...
            kixb, kixtb = self.kappai_func(
                Tfxb, self.rhox[ib0aa], self.compx[ib0aa], T4=T4
            )
            cfx[ibbaa] *= kixb
            ktx[ibbaa] = kixtb
        ...
        if self.bc[1][0] == REFLECTIVE:
            ...
            kiyb, kiytb = self.kappai_func(
                Tfyb, self.rhoy[iba1a], self.compy[iba1a], T4=T4
            )
            cfy[ibaba] *= kiyb
            kty[ibaba] = kiytb
        ...
        if self.bc[2][0] == FLUX:
            ...
            if not T4:
                dTfzb0 *= cube(Tfzb0)
            kizb0, kiztb0 = self.kappai_func(
                Tfzb0, self.rhoz[ibaa0], self.compz[ibaa0], T4=T4
            )
            cfz[ibaa0] *= kizb0
            ktz[ibaa0] = kiztb0
        ...
        if self.bc[2][1] == BLACKBODY:
            ...
            iihs = ~iibb
            kizb1, kiztb1 = self.kappai_func(
                Tfzb1[iihs], self.rhoz[ibaa1][iihs], self.compz[ibaa1][iihs], T4=T4
            )
            cfz[ibaa1][iihs] *= kizb1
            ktz[ibaa1][iihs] = kiztb1
        elif self.bc[2][1] == TEMPERATURE:
            kizb1, kiztb1 = self.kappai_func(
                Tfzb1, self.rhoz[ibaa1], self.compz[ibaa1], T4=T4
            )
            cfz[ibaa1] *= kizb1
            ktz[ibaa1] = kiztb1
        else:
    ...
    def solve(self, T, op=None, tol=1e-6, f=None, lofac=1e-3):
        ...
        for i in range(1000):
            ...
            if err0 > 1 - lofac:
                ...
                print(
                    f'[DIFFUSE.SOLVE] {i:>3d}: Limiting due to excess correction of cell {ierr}'
                )
            if opx != 1:
                T -= c * opx
            else:
            ...
            if f.keywords.get('T4', True):
                Terr = qqrt(Terr)
            print(
                f'[DIFFUSE.SOLVE] {i:>3d}: err = {err:+8e}, T = {Terr:+8e}, opx = {opx:+8e}, ierr = {ierr}'
            )
            if np.isnan(err):
    ...
    def evo_step(self, T, dT=None, dt=1e-4, t=None, maxit=20, lofac=1e-3):
        """
        ...
        dti = 1 / dt
        mti = self.m.flatten() * dti

        diag = partial(diags, format='csc')
        T4 = self.method == 'solve4'
        f = partial(self.func, T4=T4)
        cpf = partial(
            self.cp_func, rho=self.rhoc.flatten(), comp=self.compc.flatten(), T4=T4
        )
        if T4 == True:
            T0q = qqrt(T0)
        solver = spsolve
        ...
        for i in range(maxit):
            ...
            if self.method == 'solve4':
                ...
                b = F - mti * cp * dTq
                # b = F - Cv0 * (Tnq - T0q)
                A = dF - diag(mti * (0.25 * cp * Tnq / Tn + cpt * dTq))
                # A = dF - diag(Cv04 * Tnq / Tn)
            else:
                b = F - mti * cp * dT
                A = dF - diag(mti * (cp + cpt * dT))
            c = solver(A, b)
            err0 = np.max(c / T0)
            if err0 > 1 - lofac:
                opx = (1 - lofac) / (1 + err0)
            ...
            print(f'[EVO_STEP] [{i:>4}] dTr = {dTr:+8e}, dTs = {dTs:+8e}')
            if dTs < 1e-6:
                break
        else:
            return None, 0, 0
        Tn = Tn.reshape(self.shape)
        dT = dT.reshape(self.shape)
        return Tn, dT, dTr
    ...
    def evo(
        ...
        i = i0 = getattr(self, 'i', 0)
        if t is None:
            t = getattr(self, 't', 0)
        self.t = t
        if i == 0 and dt0 is not None and dt is None:
            dt = dt0
        if dt is None:
            dt = getattr(self, 'dt', 1e-4)
        dt0 = dt
        ...
        while (t < tstop) and (i < nstop):
            ...
            if (
                (cause_none := (Tn is None))
                or (cause_dTrd := (dTr_ > 2 * dTr))
                or (cause_zero := np.any(Tn <= 0))
                or (cause_nan := np.any(np.isnan(Tn)))
            ):
                dt = dt * 0.1
                if cause_none:
                    cause = 'Fail'
                    ...
                    cause = 'nan'
                print(f'[EVO] backup {cause} {i:>6d}, dt = {dt:12.5e}')
                i = i - 1
                dT = dT0
                Tn = T
                T = Tp
            ...
            dt = dtn
            dT = dT * (dt / dt0)
            dT = np.minimum(np.maximum(dT, Tn * (-1 / GOLDEN)), Tn * GOLDEN)
            use = resource.getrusage(resource.RUSAGE_SELF)
            xt[2, :] = (time.time(), use.ru_utime, use.ru_stime)
            x = xt[2] - xt[1]
            x = np.hstack((x, np.sum(x[1:])))
            print(
                f'[EVO] [{i:>6d}] time = {t:+8e}, dtn = {dt:+8e}, real: {time2human(x[0]):>8s}, CPU: {time2human(x[3]):>8s}'
            )
            self.t = t
            self.T = T_
            self.dt = dt
        ...
        x = np.hstack((x, np.sum(x[1:])))
        print(
            f'[EVO] total: {i - i0:6d} steps, time: {time2human(x[0]):>8s} real, {time2human(x[1]):>8s} user, {time2human(x[2]):>8s} sys, {time2human(x[3]):>8s} cpu total.'
        )
    ...
    def store2movie(
        ...
        fkw = dict(size=(720, 512))
        fkw = dict(fkw, **fkwargs)
        mkw = dict(fps=60, mp=mp)
        mkw = dict(mkw, **mkwargs)
        # pkw = dict(q=('eps', 'T'), plot='plot2')
        pkw = dict(q='T', vmin=10 ** 6.4, vmax=10 ** 9.5, plot='plot')
        pkw = dict(pkw, **pkwargs)
        plot = pkw.pop('plot')
        q = pkw.pop('q')
    ...
    def store2lc(self, fig=None, ax=None, label=None):
        t = np.array([x['t'] for x in self.store])
        T = np.array([x['T'][0, 0, -1] for x in self.store])
        L = self.r ** 2 * np.pi * 4 * self.SB * T ** 4
        if ax is None:
            if fig is None:
                fig = plt.figure()
            ax = fig.add_subplot()
        if fig is None:
            fig = ax.figure
        ax.plot(t, L * 1e-38, label=label)
        ax.set_xscale('timescale')
        ax.set_ylabel(r'$L_\mathrm{bol}$ ($10^{38}\,\mathrm{erg}\,\mathrm{s}^{-1}$')
        fig.tight_layout()
        self.axes = ax
        self.fig = fig

    def copy(self, pickle=False, retain_store=False):
        if not retain_store:
            try:
                del self.store
            except:
                pass
        if pickle:
            return pickle.gets(pickle.dumps(self))
        else:
            if not hasattr(self, 'axes'):
                self.axes = None
            if not hasattr(self, 'fig'):
                self.fig = None
            tmp = self.axes, self.fig
            self.axes = None
            self.fig = None
            c = deepcopy(self)
            self.axes, self.fig = tmp
            return c
    ...
    def Q(self, q=None, T=None, net=None):
        ...
        if q == 'eps':
            ...
            ki, kit = self.kappai_func(T, self.rhoc, self.compc, T4=False)
            Q = 1 / ki
            label = 'opacity'
            ...
            cp, cpt = self.cp_func(T, self.rhoc, self.compc, T4=False)
            Q = cp
            label = r'$c_\mathrm{p}$'
            unit = r'$\mathrm{erg}\,\mathrm{g}^{-1}\,\mathrm{s}^{-1}$'
            defaultscale = 'linear'
            cmap = color.colormap('magma_r')
        else:
    ...
    def plot(
        ...
        flatten=True,
        aspect='equal',
        mode=None,  # voxel, project, coord, shell, line
        loglim=10,
    ):
        ...
        if mode == 'voxel':
            ...
            coord3d = (self.xc, self.yc, self.zc)
        elif mode in ('coord',):
            ...
            if mode in ('project',):
                if self.geo == 'patch':
                    if ix is not None:
                        ...
                        ) - sqrt(np.sum(self.vz0[[0, 2]] ** 2, axis=-1))
                        coord3d[0] = 0
                    elif iy is not None:
                        coord3d[2] = sqrt(
                            np.sum((self.vye + self.vz0)[..., [1, 2]] ** 2, axis=-1)
                        ) - sqrt(np.sum(self.vz0[[1, 2]] ** 2, axis=-1))
                        coord3d[1] = 0
                    ...
                    if iy is not None:
                        ...
                        ) - sqrt(np.sum(self.vz0[[0, 1]] ** 2, axis=-1))
                    elif ix is not None:
                        v = self.vxe + self.vz0
                        ra = sqrt(np.sum(v[..., [0, 1]] ** 2, axis=-1))
                        rc = sqrt(np.sum(v[..., [0, 1, 2]] ** 2, axis=-1))
        ...
        if scale == 'log':
            ...
            vmin = np.maximum(vmin, vmax - loglim)
        elif scale == 'sci':
            norm = LogNorm
        elif mode in ('line',):
            qscale = np.floor(np.log10(np.max(Q)))
            if np.abs(qscale) > 2:
                f = 1 / 10 ** qscale
                Q = Q * f
                vmin = vmin * f
                vmax = vmax * f
                qpow = f'$10^{{{int(qscale):d}}}$'
                if unit is None:
                    unit = qpow
                elif qpow.endswith('$') and unit.startswith('$'):
                    unit = qpow.rstrip('$') + '\,' + unit.lstrip('$')
                else:
                    unit = qpow.rstrip('$') + '\,$' + unit
            norm = None
        else:
            norm = Normalize
        if unit is not None:
        ...
        if mode in ('line',):
            span = vmax - vmin
            lim = np.array([vmin, vmax]) + 0.05 * span * np.array([-1, 1])
            ax.set_ylim(lim)
        if ylim is not None:
            ax.set_ylim(ylim)
        fig.text(0.01, 0.99, self.geo + '\n' + mode, ha='left', va='top')
        ...
        Thot = self.thot
        with open(
            f"data/data_solutiontemp_runsubstrate_{run}_qb{qb}_hs{Thot}_2minsbefore.txt", "wb",
        ) as f:
            np.savetxt(
                f,
                Q.T,
        ...
        # kap, label, unit, defaultscale, cmap = self.Q(q='k', T=T, net=net)
        # print(np.shape(kap[:,0,:]))
        # with open(
        #     f"data/data_solutiontemp_runsubstrate_{run}_qb{qb}_hs{Thot}_kappa_cart.txt",
        #     "wb",
        # ) as f:
        #     np.savetxt(
        #         f,
        ...
        #         fmt="%.18e",
        #         delimiter=" ",
        #         header="kappa[0,:], .. kappa[x,:]",
        #         comments="# everything is in cgs\n",
        #     )
        # eps, label, unit, defaultscale, cmap = self.Q(q='eps', T=T, net=net)
        # with open(
        #     f"data/data_solutiontemp_runsubstrate_{run}_qb{qb}_hs{Thot}_epsnuc_cart.txt",
        #     "wb",
        # ) as f:
        #     np.savetxt(
        #         f,
        ...
        #         fmt="%.18e",
        #         delimiter=" ",
        #         header="epsnuc[0,:], .. epsnuc[x,:]",
        #         comments="# everything is in cgs\n",
        #     )

    def plotly(self, q='T', T=None, net=None, scale=None, image=True, fig=None):
