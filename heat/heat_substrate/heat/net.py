...
class Net(object):
    ...
    def __init__(self, ppn, shape=None):
        if shape is None:
            self.ppn = ppn
        else:
    ...
    def eps(self, tem, den):
        return np.sum(self.q * self.rate(tem, den).reshape(tem.shape + self.q.shape), axis=-1)
    ...
    def enuc(self):
        return np.sum(self.me * self.ppn, axis=-1)
...
class Net3aFFNs(Net):
    ...
    q = np.array([7.274 * 1.602176634e-06 * 6.02214076e+23])
